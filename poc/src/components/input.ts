...
export class OrgInput extends LitElement {
  ...
  @property({ type: String }) type: 'text' | 'email' | 'password' | 'number' | 'search' | 'tel' | 'url' = 'text';
  ...
  private get _wrapperClass() {
    ...
    if (this.success && !this.error)  classes.push('success');
    if (this.disabled) classes.push('disabled');
    if (this._hasPrefix) classes.push('has-prefix');
    if (this._hasSuffix) classes.push('has-suffix');
    return classes.join(' ');
  }
...
declare global {
  interface HTMLElementTagNameMap { 'org-input': OrgInput; }
}
